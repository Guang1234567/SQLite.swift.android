...
public class SQLiteBase {
    ...
    public func open(dbPath: String) -> Bool {
        if db != nil {
            closeDB()
        }
        ...
        let error = sqlite3_open_v2(cpath!, &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_NOMUTEX | SQLITE_OPEN_SHAREDCACHE, nil)
        if error != SQLITE_OK {
    ...
    public func closeDB() {
        if db != nil {
            if CInt(sqlite3_close_v2(db)) == SQLITE_OK {
                NSLog("SQLiteDB - Success cleaning DB")
            } else {
